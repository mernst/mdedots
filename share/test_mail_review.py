"""Tests for the mail-review scripts that do not access Gmail or OAuth credentials."""
...
import mail_review  # ruff: ignore[module-import-not-at-top-of-file]


def load_script(file_name: str) -> types.ModuleType:
    """Import one of the review scripts, whose name is not a legal module name.

    Returns:
        The imported module.
    """
    module_name = Path(file_name).stem.replace("-", "_")
    loader = importlib.machinery.SourceFileLoader(module_name, str(SCRIPT_DIRECTORY / file_name))
...
mail_review_spam = load_script("mail-review-spam.py")
mail_review_weekly = load_script("mail-review-weekly.py")

REVIEWS = (
    (mail_review_spam.SPAM_REVIEW_SECTIONS, mail_review_spam.SPAM_REVIEW),
    (mail_review_weekly.WEEKLY_REVIEW_SECTIONS, mail_review_weekly.WEEKLY_REVIEW),
)
...
def session(email, slot, result=None, error=None):
    ...
    config = mail_review.AccountConfig(email, Path("unused-token.json"))
    return mail_review.AccountSession(config, slot, FakeService(result, error))
...
class MailReviewTest(unittest.TestCase):
    """Test API queries and browser-opening decisions."""

    @mock.patch.object(mail_review.time, "sleep")
    @mock.patch.object(mail_review.webbrowser, "open")
    def test_no_accounts_match_opens_no_tabs(self, browser_open, _sleep):
        ...
        mail_review.open_search("subject:none", sessions, mail_review_spam.SPAM_REVIEW)
    ...
    def test_only_matching_account_opens_encoded_url(self, browser_open, _sleep):
        ...
        mail_review.open_search(
            'subject:"Müller test" to:a@example.com', sessions, mail_review_spam.SPAM_REVIEW
        )
    ...
    @mock.patch.object(mail_review.time, "sleep")
    @mock.patch.object(mail_review.webbrowser, "open")
    def test_review_without_prefix_searches_the_term_itself(self, browser_open, _sleep):
        sessions = (session("one@example.com", "0", {"messages": [{"id": "1"}]}),)
        mail_review.open_search(
            "label:delay-weekly seworld", sessions, mail_review_weekly.WEEKLY_REVIEW
        )
        browser_open.assert_called_once_with(
            "https://mail.google.com/mail/u/0/#search/label%3Adelay-weekly%20seworld"
        )
    ...
    def test_both_accounts_match_in_account_order(self, browser_open, _sleep):
        ...
        mail_review.open_search("(from:a OR from:b)", sessions, mail_review_spam.SPAM_REVIEW)
        self.assertEqual(
            [call.args[0].split("/u/")[1][0] for call in browser_open.call_args_list],
            ["0", "2"],
        )

    def test_spam_api_request_checks_one_spam_message(self):
        service = FakeService({"messages": [{"id": "1"}]})
        query = mail_review_spam.SPAM_REVIEW.query('subject:"hello"')
        self.assertTrue(
            mail_review.search_succeeds(
                service, query, mail_review_spam.SPAM_REVIEW.include_spam_trash
            )
        )
        self.assertEqual(
    ...
    def test_weekly_api_request_ignores_spam_and_trash(self):
        service = FakeService({"messages": [{"id": "1"}]})
        query = mail_review_weekly.WEEKLY_REVIEW.query("label:delay-bulk")
        self.assertTrue(
            mail_review.search_succeeds(
                service, query, mail_review_weekly.WEEKLY_REVIEW.include_spam_trash
            )
        )
        self.assertEqual(
            service.messages_resource.list_arguments,
    ...
    def test_browser_account_positions_become_slots(self):
        ...
        self.assertEqual(
            mail_review.parse_browser_accounts(payload),
    ...
    def test_changed_browser_response_fails_loudly(self):
        with self.assertRaisesRegex(ValueError, "not valid base64"):
            mail_review.parse_browser_accounts("obsolete JSON response")
    ...
    def test_missing_expected_browser_account_requires_rewrite(self):
        ...
        with (
            ...
            mail_review.discover_browser_slots(("expected@example.com",))
    ...
    def test_browser_account_request_matches_chromium(self):
        ...
        self.assertEqual(calls[0][0], (mail_review.LIST_ACCOUNTS_URL,))
        self.assertEqual(
            calls[0][1],
            {
                "data": " ",
    ...
    def test_transient_error_opens_tab_with_warning(self, browser_open, _sleep):
        with contextlib.redirect_stderr(io.StringIO()) as stderr:
            mail_review.open_search(
                "subject:test",
                (session("one@example.com", "0", error=http_error(503)),),
                mail_review_spam.SPAM_REVIEW,
            )
    ...
    def test_permanent_error_stops_without_opening_tab(self, browser_open):
        with self.assertRaisesRegex(RuntimeError, "Gmail search failed for one@example.com"):
            mail_review.open_search(
                "subject:test",
                (session("one@example.com", "0", error=ValueError("bad")),),
                mail_review_spam.SPAM_REVIEW,
            )
        browser_open.assert_not_called()
    ...
    def test_token_file_is_private(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            mail_review._write_private_token(token_path, "secret")
            self.assertEqual(token_path.read_text(encoding="utf-8"), "secret")
            self.assertEqual(stat.S_IMODE(token_path.stat().st_mode), 0o600)

    def test_spam_folder_url(self):
        self.assertEqual(
            mail_review.gmail_folder_url("2", "spam"), "https://mail.google.com/mail/u/2/#spam"
        )

    @mock.patch.object(mail_review.time, "sleep")
    @mock.patch.object(mail_review.webbrowser, "open", return_value=False)
    def test_unopened_tab_warns(self, _browser_open, _sleep):
        with contextlib.redirect_stderr(io.StringIO()) as stderr:
            mail_review.open_tab("https://mail.google.com/mail/u/0/#spam")
...
class TransientErrorTest(unittest.TestCase):
    ...
    def test_server_and_throttling_errors_are_transient(self):
        for status in (408, 429, 500, 503):
            self.assertTrue(mail_review.is_transient_api_error(http_error(status)), status)

    def test_rate_limited_forbidden_is_transient(self):
        self.assertTrue(
            mail_review.is_transient_api_error(http_error(403, "userRateLimitExceeded"))
        )

    def test_unauthorized_forbidden_is_permanent(self):
        self.assertFalse(
            mail_review.is_transient_api_error(http_error(403, "insufficientPermissions"))
        )
        self.assertFalse(mail_review.is_transient_api_error(http_error(400, "badRequest")))

    def test_network_failures_are_transient(self):
        self.assertTrue(mail_review.is_transient_api_error(ConnectionResetError("reset")))
        self.assertTrue(mail_review.is_transient_api_error(TimeoutError("timed out")))
...
class CachedCredentialsTest(unittest.TestCase):
    ...
    def cached_token(self, from_authorized_user_file):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            config = mail_review.AccountConfig("one@example.com", token_path)
    ...
    def test_absent_cache_requires_authorization(self):
        config = mail_review.AccountConfig("one@example.com", Path("no-such-token.json"))
        self.assertIsNone(mail_review.cached_credentials(config))
    ...
    def test_valid_cache_is_reused(self):
        ...
        with self.cached_token(lambda _path, _scopes: credentials) as config:
            self.assertIs(mail_review.cached_credentials(config), credentials)
...
class SectionDataTest(unittest.TestCase):
    """Test the review sections that each script works through."""

    def all_searches(self, sections):
        """Return every search term of a review, in review order."""
        return [search for section in sections for search in section.searches]

    def test_every_section_has_a_heading_and_searches(self):
        for sections, _review in REVIEWS:
    ...
    def test_no_search_is_repeated(self):
        for sections, _review in REVIEWS:
            searches = self.all_searches(sections)
    ...
    def test_searches_have_balanced_parentheses_and_quotes(self):
        for sections, _review in REVIEWS:
            for search in self.all_searches(sections):
                self.assertEqual(search.count("("), search.count(")"), search)
                self.assertEqual(search.count('"') % 2, 0, search)

    def test_searches_are_not_url_encoded(self):
        # Gmail queries are written in plain text; `gmail_search_url` encodes them.
        for sections, _review in REVIEWS:
            for search in self.all_searches(sections):
