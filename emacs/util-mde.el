...
;; (cl-assert (equal 4.5 (median 1 2 3 4 5 6 7 8)))
;; (cl-assert (equal 5.0 (median 1 2 3 4 5 6 7 8 9)))
;; (cl-assert (equal 4.5 (median 4 6 7 1 5 3 8 2)))
;; (cl-assert (equal 5.0 (median 4 6 7 2 8 1 9 5 3)))
...
(defun join (strings separator)
  "Obsolete; use `string-join` instead (as of Emacs 25)."
  (declare (obsolete 'string-join "Emacs 25"))
  (let ((with-delims))
    (while strings
...
;; (cl-assert (equal (string-join '("foo" "bar") " ") "foo bar"))
;; (cl-assert (equal (string-join '("foo" "bar" "baz") " ") "foo bar baz"))
;; (cl-assert (equal (string-join '("foo") " ") "foo"))
;; (cl-assert (equal (string-join '("foo" "bar" "baz") "") "foobarbaz"))
;; (cl-assert (equal (string-join '() " ") ""))
...
When called non-interactively, requires two arguments, the region's beginning
and end."
  (interactive "r")
  (save-excursion
...
(defun how-many-string (string &optional rstart rend interactive)
  "Return number of matches for STRING following point."
  (interactive
   (keep-lines-read-args "How many matches for string"))
  (if (called-interactively-p 'any)
      (funcall-interactively #'how-many (regexp-quote string) rstart rend interactive)
    (how-many  (regexp-quote string) rstart rend interactive)))
...
(defun how-many-in-string (regexp target)
  ...
  (cl-loop with start = 0
           for count from 0
           while (string-match regexp target start)
...
(defun t-function (&rest _args)
  t)

(defun nil-function (&rest _args)
  nil)
