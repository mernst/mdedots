...
If the symbol `unidiff', then use unidiff output format instead.")
...
A T filename argument specifies the visited file and the buffer
...
(defun bdiff-backup-file (filename)
  ...
  (let* ((default-directory (file-name-directory filename))
	 ...
	 (highest 0)
	 (mname (make-backup-file-name fn))
	 (mtime (let ((attr (file-attributes mname)))
	          (if attr
		      (nth 5 attr)
	            (setq mname nil)
	            '(0 0)))))
