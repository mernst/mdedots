...
;; It is no good to use variables `kill-emacs-query-functions' or
;; `confirm-kill-emacs', because `save-buffers-kill-emacs' first saves
;; buffers, then kills processes, then runs
;; `kill-emacs-query-functions', and only at the very end checks
;; `save-buffers-kill-emacs'.
;; I wish to be queried before killing off processes, etc.
(defun save-buffers-kill-emacs--confirm (orig-fun &rest args)
  "Confirm that the user wishes to kill this Emacs process."
  (if (y-or-n-p "Kill Emacs? ")
      (apply orig-fun args)))
(advice-add 'save-buffers-kill-emacs :around #'save-buffers-kill-emacs--confirm)
...
Arbitrary BUFFER may be supplied (defaults to *grep*)."
  ...
  (with-current-buffer buffer
    ...
    (let ((buffer-read-only nil))
      ...
      (delete-matching-lines "^[-./a-z0-9_]+\\.[gs]fasl42:") ; compiled Lisp files
      (delete-matching-lines "\\bdocs/tutorial/webpages/bootstrap/js/") ; Bootstrap JavaScript
...
(defun do-grep-clean-maybe (buffer _finish-description)
...
(defun find-file--trim (args)
  "Remove leading and trailing whitespace in filename.
Such whitespace can be caused by cut and paste."
  (setcar args (string-trim (car args)))
  args)
(advice-add 'find-file :filter-args #'find-file--trim)
...
(defun find-repeated-words ()
  "Find duplicated/repeated/doubled words, such as \"the the\", in TAGS table."
  (interactive)
  (tags-search "\\b\\(\\w+\\)[[:blank:]]+\\1\\b"))
...
(defun barf-if-buffer-read-only--make-buffer-modifiable (orig-function &rest args)
  "Let the user make the current read-only buffer be writeable."
  ;; This is defined as around advice so that it only runs if
  ;; `barf-if-buffer-read-only' would issue an error, not in the many
  ;; places `barf-if-buffer-read-only' is called but issues no error.
  (condition-case e
...
(autoload 'delete-matching-paragraphs "replace-paragraphs"
  "Delete paragraphs containing matches for REGEXP." t)
(autoload 'keep-paragraphs "replace-paragraphs"
  "Delete paragraphs containing matches for REGEXP." t)
(autoload 'delete-non-matching-paragraphs "replace-paragraphs"
  "Delete paragraphs containing matches for REGEXP." t)
...
;; ;; (cl-assert (string-equal (try-completion-tester) "michael ernst"))
...
;;               (setq done t)
;;             (progn
;;               (setq result (member (cl-second frame) functions))
;;               (setq done result))))
;;         (setq i (1+ i)))
;;       result)))
...
(defun paragraph-buffer-p ()
  "Return true if the current buffer is paragraph-oriented."
  (member buffer-file-name
          (list
           (expand-file-name "~/private/addresses.tex")
...
(defun quote-for-shell-command (arguments)
  "Return ARGUMENTS, a whitespace-separated argument list, quoted for the shell.
Each whitespace-separated word of ARGUMENTS becomes one shell argument, so
quotation marks that the user typed are treated as literal characters
rather than as shell syntax."
  (mapconcat #'shell-quote-argument (split-string arguments) " "))
...
(defun shell-command-on-region--not-in-echo-area (orig-fun &rest args)
...
command, so it is convenient to have that buffer displayed."
  (let ((resize-mini-windows nil))
    (apply orig-fun args)))
(advice-add 'shell-command-on-region :around #'shell-command-on-region--not-in-echo-area)
...
(defun tags-case-fold-search ()
  "Toggle the value of variable `tags-case-fold-search'."
  (interactive)
...
This enables appending a new table to the current list."
  (interactive)
  (setq tags-add-tables 'ask-user)
  (message "`tags-add-tables' now has value 'ask-user"))

;; Window title (shorten system name, etc.)
