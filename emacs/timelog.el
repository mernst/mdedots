...
(defvar timelog-file "~/private/timelog"
...
(defun timelog ()
  ...
  (if (looking-back "[0-9][0-9][0-9][0-9]\n" (- (point) 5))
      (backward-char))
  (if (not (looking-back (rounded-current-time-string) nil))
      (progn
...
(defun rounded-current-time-string ()
  (let* ((decoded (decode-time (current-time)))
         (minutes (cl-second decoded))
         (minutes-rounded (* 15 (round minutes 15)))
         (hour (cl-third decoded)))
  ...
  (let ((first-day nil)
    ...
    (while (< (point) end)
      (cond
       ...
       ((looking-at "\\([012][0-9]\\)\\([0-5][05]\\)-\\([012][0-9]\\)\\([0-5][05]\\) \\([RAESLMYZ]\\) \\(.*\n\\)")
        (let* ((start-hour (+ (string-to-number (match-string 1))
          ...
          (cond ((equal category "R")
                ...
                ((equal category "A")
                 ...
                 (setq advising-time (+ advising-time duration))
                 (push detail advising-lines))
                ((equal category "E")
                 ;; education
                 (setq education-time (+ education-time duration))
                 (push detail education-lines))
                ((equal category "S")
                 ;; service; includes undergraduate advising
                 (setq service-time (+ service-time duration))
                 (push detail service-lines))
                ((equal category "L")
                 ;; logistics
                 (setq logistics-time (+ logistics-time duration))
                 (push detail logistics-lines))
                ((equal category "M")
                 ;; miscellaneous:  non-work
                 (setq misc-time (+ misc-time duration))
                 (push detail misc-lines))
                ((equal category "Y")
                 ;; special Y category:  for example, for consulting
                 (setq y-time (+ y-time duration))
                 (push detail y-lines))
                ((equal category "Z")
                 ;; special Z category:  for example, for consulting
                 (setq z-time (+ z-time duration))
                 (push detail z-lines))
                (t
    ...
    (let ((after-summary (point)))
      ...
      (if advising-lines
          (apply #'insert "\nAdvising:\n" (sort advising-lines)))
      (if education-lines
          (apply #'insert "\nEducation:\n" (sort education-lines)))
      (if service-lines
          (apply #'insert "\nService:\n" (sort service-lines)))
      (if logistics-lines
          (apply #'insert "\nLogistics:\n" (sort logistics-lines)))
      (if misc-lines
          (apply #'insert "\nMiscellaneous:\n" (sort misc-lines)))
      (if y-lines
          (apply #'insert "\nY:\n" (sort y-lines)))
